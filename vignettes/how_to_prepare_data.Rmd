...
You can read in FCS files using the `flowCore` package available from Bioconductor
and convert it to a `data.table` object.
...
The code above used `flowCore`'s `read.flowSet` function to first read FCS files into
a `flowSet` object.

`lapply` and `rbindlist` is then used to convert it to one `data.table` object containing
data from all FCS files.
...
[article on data transformation](https://immunedynamics.io/spectre/tutorials/data_transformation/data_transformation.html).
...
[article](https://immunedynamics.io/spectre/tutorials/data_transformation/data_transformation.html).
...
```{r}
markers_to_transform <- c(
  "209Bi_CD11b", "162Dy_CD11c", "163Dy_CD7", "166Er_CD209", "167Er_CD38", "151Eu_CD123",
  "153Eu_CD62L", "152Gd_CD66b", "154Gd_ICAM-1", "155Gd_CD1c", "156Gd_CD86", "160Gd_CD14", "165Ho_CD16",
  "191Ir_DNA1", "193Ir_DNA2", "175Lu_PD-L1", "142Nd_CD19", "146Nd_CD64", "195Pt", "196Pt",
  "198Pt_Dead", "147Sm_CD303", "148Sm_CD34", "149Sm_CD141", "150Sm_CD61", "169Tm_CD33", "89Y_CD45",
  "170Yb_CD3", "173Yb_CD56", "174Yb_HLA-DR"
)
```
...
```{r}
new_cols <- paste0(markers_to_transform, "_asinh")
cofactor <- 5
dat[, (new_cols) := lapply(.SD, function(x) asinh(x / cofactor)), .SDcols = markers_to_transform]
```
