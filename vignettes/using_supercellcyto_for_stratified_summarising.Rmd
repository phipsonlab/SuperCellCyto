...
biological sample the cell came from, but rather` the column the we want to stratify 
the data based on.

For example, when drawing UMAP or tSNE plot, we commonly subsample each cluster or 
cell type to avoid crowding the plot.
...
`n_cells_in_the_cluster/20` where 20 is the `gam` parameter value we used for 
